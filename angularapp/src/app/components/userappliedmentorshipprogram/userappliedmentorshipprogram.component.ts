...
export class UserappliedmentorshipprogramComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.userId) {
      ...
      Swal.fire({
        title: 'Fetching Applied Applications...',
        text: 'Please wait while we load your applied mentorship programs.',
      ...
      this.mentorshipService.getAppliedMentorshipPrograms(this.userId).subscribe((data: any[]) => {
        ...
        Swal.close();
        Swal.fire('Error', 'Failed to fetch applied mentorship programs. Please try again later.', 'error');
